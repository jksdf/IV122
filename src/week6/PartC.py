...
def weed(fn):
    ...
    lsystem.run(turtle, 1, 6, 25)
    turtle.resetpos((100, 0), -90)
    lsystem.run(turtle, 2, 6, 25)
    turtle.resetpos((150, 0), -90)
    lsystem.run(turtle, 3, 6, 25)
    turtle.resetpos((200, 0), -90)
    lsystem.run(turtle, 4, 6, 25)
    turtle.resetpos((300, 0), -90)
    lsystem.run(turtle, 5, 6, 25)
...
class PartC(Base):
    ...
    def run(self, fnprovider: AbstractFilenameProvider):
        weed(fnprovider.get_filename('.svg', 'test', 'test'))
